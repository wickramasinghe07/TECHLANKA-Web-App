...
const userController = {
  ...
  createUser: async (req, res) => {
    try {
      ...
      if (!firstName || !lastName || !role || !phoneNumber || !password || !email)
  ...
  updateUser: async (req, res) => {
    try {
      ...
      await User.findOneAndUpdate(
        { _id: id },
        {firstName, lastName,role, phoneNumber, email, password }
      );
      res.json({
        message: "User update success",
        data: {firstName, lastName,role, phoneNumber, email, password },
      });
    } catch (err) {
      return res.status(500).json({ message: err.message });
    }
